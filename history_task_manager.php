...
$previous_step=mysql_result($load_header,0,4)."|".datecs(mysql_result($load_header,0,5))."|".datecs(mysql_result($load_header,0,14))."|".mysql_result($load_header,0,15)."|".mysql_result($load_header,0,3)."|".mysql_result($load_header,0,13)."|".@$laststatus."|".mysql_result($load_header,0,16);

$htmlcode.='<SPAN style="width:100%;height:100%;" ><table style="width:100%;height:100%;border:0px;border-collapse:collapse;" ><tr style="width:100%;border:0px;background-color:#C5E0E6;" ><td>'.dictionary("title",$_SESSION["language"]).'</td><td style="text-align:left;width:100%;" ><input type="text" id="hist_value1" name="hist_value1" value="'.mysql_real_escape_string(mysql_result($load_header,0,1)).'" style="width:80%;font-weight:bold;" disabled="disabled" ></td><td style="text-align:center;" ><font style="font-size:10px;" >'.dictionary("request_to",$_SESSION["language"]).'</font><br /><input id="hist_value2" disabled="disabled" name="hist_value2" type="text" value="'.datecs(mysql_result($load_header,0,5)).'" style="width:100px;text-align:center;font-weight:bold;" ></td></tr><tr style="width:100%;border:0px;background-color:#C5E0E6;" ><td colspan="3" width="100%" ><table style="width:100%;" ><tr align="middle" ><td style="width:25%;" >'.dictionary("priority",$_SESSION["language"]).'<BR /><select size="1" name="hist_value3" disabled="disabled" >';

    $load_form_data=mysql_query("select param from mainsettings where name=\"task_manager_priorities\" ") or die (dictionary("sql_command",$_SESSION["language"])." > ".MySQL_Error());
...
$htmlcode.='</td><td style="width:25%;text-align:middle;" >'.dictionary("status",$_SESSION["language"]).'<BR /><select '.access("1/2off").' size="1" name=hist_value4 disabled >';
    $load_form_data=mysql_query("select param from mainsettings where name=\"task_manager_statuses\" ") or die (dictionary("sql_command",$_SESSION["language"])." > ".MySQL_Error());
    $part=explode(",",mysql_result($load_form_data,0,0));
    $cycle=0;while($part[$cycle]<>""):
    $htmlcode.="<option value=\'".$part[$cycle]."\'";
...
$load_form_data=mysql_query("select * from task_manager_attachment where parent_no='".securesql(@$_GET["id"])."' ") or die (dictionary("sql_command",$_SESSION["language"])." > ".MySQL_Error());
    ...
    $htmlcode.='<a style="border:0px;" href="./ajax_functions.php?show_file=yes&tbl=task_manager_attachment&id='.@mysql_result($load_form_data,$cycle,0).'" target=_blank ><image src="./images/attachment.png" border=0 width=24px height=24px title="'.@mysql_result($load_form_data,$cycle,2).'\r\n'.datetimedb_to_datecs(@mysql_result($load_form_data,$cycle,5)).'" /></a> ';
    $cycle++;endwhile;
...
$htmlcode.="<div style=\"position:relative;left:0px;bottom:0px;height:100%;text-align:center;\" ><font style=\"font-size:10px;\" ><span title=\"".dictionary("request_to",$_SESSION["language"])."\" >".datecs(mysql_result($load_data,$cycle,6))."<br/><span title=\"".dictionary("solution_to",$_SESSION["language"])."\" >".datetimedb_to_datecs(mysql_result($load_data,$cycle,14))."</span><br/><span title=\"".dictionary("priority",$_SESSION["language"])."\" >".mysql_result($load_data,$cycle,5)."</span><br/><span title=\"".dictionary("score",$_SESSION["language"])."\" >".mysql_result($load_data,$cycle,13)."</span><br/><span title=\"".dictionary("status",$_SESSION["language"])."\" >".mysql_result($load_data,$cycle,4)."</span><br/><span title=\"".dictionary("create_date",$_SESSION["language"])."\" >".datetimedb_to_datecs(mysql_result($load_data,$cycle,10))."</span><br/><span title=\"".mysql_result($load_data,$cycle,15)."\" >".dictionary("solves",$_SESSION["language"])."</span></font></div>";
...
if ((strpos(" ".@mysql_result($load_header,0,15),','.@$_SESSION["lnamed"].',') or @mysql_result($load_header,0,10)==@$_SESSION["lnamed"] or $_SESSION['sysadmin']=="Yes" ) && @mysql_result($load_header,0,3)<>@$laststatus){?>document.getElementById('addstep').disabled=false;<?}
    else {?>document.getElementById('addstep').disabled=true;<?}
?>
